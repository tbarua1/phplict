...
$layout->version = 3;
	$layout->bootstrapTheme = "cerulean";
		$layout->customCssPageName = "district_detailspreview";
...
$page_layouts["district_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	dname - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("dname", $data, $keylink);
			$row["dname_value"] = $value;
			$format = $pSet->getViewFormat("dname");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("dname")))
				$class = ' rnr-field-number';
			$row["dname_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("district", "detailspreview"));
